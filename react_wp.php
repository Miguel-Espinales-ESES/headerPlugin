<?php
/*
Plugin Name: wpHeader Plugin
Description: Use the [wpHeader] shortcode to display the plugin
Version: 0.0.1
...
class wpHeader {
    ...
    */
    ...
    add_action('wp_enqueue_scripts', [$this, 'custom_header_scripts'], 0 , 0 );
    }
    ...
    function add_type_attribute_front($tag, $handle, $src) {
        ...
        if ($handle  === 'wpHeader-plugin-dev' || $handle  === 'wpHeader-plugin-prod') {
            $tag = '<script type="module" src="' . esc_url($src) . '"></script>';
            return $tag;
        } else if ($handle  === 'wpHeader') {
            $tag = '<script type="module" src="' . esc_url($src) . '"></script>';
            return $tag;
        }
    ...
    {   
        add_filter('script_loader_tag', [$this, 'add_type_attribute_front'], 0, 3);
        $name = 'wpHeader';
        ...
        wp_register_script( $name, '', [], false, ['in_footer' => false] );
        wp_enqueue_script( $name );
    ...
    function api_init() {
        register_rest_route('wpHeader/v1', '/user/', [
            'methods'   => 'GET',
    ...
    */
    function init() {
        add_shortcode( 'wpHeader', [$this, 'plugin_shortcode'] );
